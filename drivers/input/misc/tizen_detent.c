...
static int hall_sensor_power_on(struct hall_sensor_driverdata *ddata)
{
	...
	if (pdata->ldo_name) {
		...
		dev_info(&input_dev->dev, "%s: %s power on", __func__, pdata->ldo_name);
	}
...
static int hall_sensor_power_off(struct hall_sensor_driverdata *ddata)
{
	...
	if (pdata->ldo_name) {
		...
		dev_info(&input_dev->dev, "%s: %s power off", __func__, pdata->ldo_name);
	}
...
static int hall_sensor_resume(struct device *dev)
{
	...
	dev_dbg(dev, "%s: resume_state=[%d]\n", __func__, ddata->resume_state);
